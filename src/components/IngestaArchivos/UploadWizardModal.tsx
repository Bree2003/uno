...
// --- NOVEDAD: Importamos nuestros services ---
import * as storageService from "services/storage-old";
import * as loggingService from "services/logging"; // Asumiendo que tienes un loggingService
...
export default function UploadWizardModal({
  ...
  useEffect(() => {
    if (initialData) {
      // --- NOVEDAD: Inyectamos los datos que ya tenemos en el paso 1 ---
  ...
  const fetchStepData = async (step: number) => {
    ...
    try {
      const response = await fetch(
        "https://data-products-backend-dev-697719423009.us-east4.run.app/api/storage/analyze",
        {
          method: "POST",
          body: formData,
        }
  ...
  const handleFinalIngest = async () => {
    ...
    try {
      ...
      await sendLog(
        "error",
        `Error al cargar archivo ${file?.name}: ${error.message}`, datasetName, user, productName
      );
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50 p-4">
      <div className="bg-[--color-gris-claro] rounded-xl shadow-2xl p-8 w-full max-w-3xl h-[540px] flex flex-col">
        <Stepper isNuevo={false} currentStep={currentStep} />
        <div className="flex-grow mt-4">
          {/* --- NOVEDAD: Pasamos los datos del paso 1 (ya enriquecidos) --- */}
